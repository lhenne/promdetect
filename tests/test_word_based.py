...
class FeatureExtractionTests(unittest.TestCase):
    def test_duration_extraction(self):
        ...
        true_dur_vals = np.around(tester.features.iloc[12:18]["dur"].to_numpy(dtype=float), decimals=4)

        expected_dur_normed_vals = np.array(
            [0.880281, 0.741253, 1.922994, 0.648567, 0.602224, 1.204681]
        )
        true_dur_normed_vals = np.around(tester.features.iloc[12:18]["dur_normed"].to_numpy(dtype=float), decimals=6)
