...
def getTranscriptData(transcriptFile):
    ...
    rawContent = rawContent.replace("    ", "") # Eliminate the indent before each line
    content = StringIO(rawContent) # Convert to a StringIO object that can be parsed by the `pandas.read_csv` function
    transcriptData = pd.read_csv(content, sep=" ", engine="python" , quoting=3)
